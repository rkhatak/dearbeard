...
{
	$sql_countproduct= "SELECT *FROM product WHERE status = 'Publish'";
...
function user_info($con,$useremail)
{
	$sql_user= "SELECT *FROM users WHERE UserEmail = '$useremail'";
	$run_user = mysqli_query($con,$sql_user) or die(mysqli_error($con));
	$userinfo = mysqli_fetch_array($run_user);
	return $userinfo;
}



function order_info($con,$UserId)
{
	$sql_order= "SELECT *FROM orders WHERE custemer_id = '$UserId'";
	$run_order = mysqli_query($con,$sql_order) or die(mysqli_error($con));
	return $run_order;
...
function update_users($con,$id,$user_editinfo)
{
	$sql = "UPDATE users SET" ;
	foreach($user_editinfo as $key=>$value)
	{
	 $sql = $sql." $key = '".$value."' , " ;	 
	}
	$sql_main = rtrim($sql," ,")." WHERE UserID='$id'";
...
function update_ship($con,$id,$user_info)
{
	$sql = "UPDATE users_shiping SET" ;
	foreach($user_info as $key=>$value)
	{
	 $sql = $sql." $key = '".$value."' , " ;	 
	}
...
function update_bill($con,$id,$user_info)
{
	$sql = "UPDATE users SET" ;
	foreach($user_info as $key=>$value)
	{
